...
function haversineDistance(loc1, loc2) {
  ...
  if (request.status === 'pending') {
    if (status === 'accepted') {
      ...
      // اگر towInfo فرستاده شده، ذخیره کن
      if (towInfo) request.towInfo = towInfo;
      clearTimeout(request.timeout);
      requests.set(requestId, request);

      // اطلاع به راننده: شامل اطلاعات یدک‌کش
      io.to(request.driverSocketId).emit('requestUpdate', { 
        requestId, 
        status: 'accepted', 
        towId: socket.id, 
        towInfo: request.towInfo || (tows.get(socket.id) && tows.get(socket.id).info) || null 
      });
      ...
      // request همچنان pending می‌ماند
    } else {
